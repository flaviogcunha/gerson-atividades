...
const CadastroPets: React.FC = () => {
    ...
    async function getClientes() {
        const empresas: THttpResponse<Cliente> = await fetch(
            "http://localhost:8080/cliente/all",
            { method: "GET" }
        ).then((response) => response.json());

        setClientes(empresas.Data as Array<Cliente>);
    }
    ...
    async function submitForm(event: React.FormEvent<HTMLFormElement>) {
        ...
        const httpResponse: THttpResponse<any> = await fetch(
            "http://localhost:8080/pet/new",
            {
                method: "POST",
        ...
        <form className="container mb-5" onSubmit={(e) => submitForm(e)}>
            <fieldset>
                ...
                <div className="mb-3">
                    ...
                    />
                    <div id="racaHelp" className="form-text">
                        Insira a raça do Pet.
                    </div>
                </div>
                <div className="mb-3">
                    ...
                    <select
                        defaultValue=""
                        onChange={(e) => setInfo({ ...info, genero: e.currentTarget.value })}
                    ...
                    <select
                        ...
                        onChange={(e) =>
                            setInfo({
                                ...
                                cliente: clientes.find((x) => x.id === +e.currentTarget.value)!,
                            })
                        }
                        className="form-select"
                    >
                        ...
                        {clientes.map((cliente, key) => (
                            <option key={key} value={cliente.id}>
                                {cliente.nome + " | " + cliente.cpf.valor}
                            </option>
                        ))}
                    </select>
                    <div id="clienteHelp" className="form-text">
                        Selecione um cliente.
                    </div>
                </div>
            </fieldset>
            <div className="row justify-content-end">
                <button
                    type="submit"
                    className="col-md-3 btn btn-success custom-btn"
                    style={{ backgroundColor: "#FF0000", color: "#FFFFFF", width: "160px" }}
                >
                    Salvar alterações
                </button>
            </div>
